...
- (void)viewDidLoad {
   ...
   // 适配ios7
	...
	_startCityText.text = @"临安";
	_startAddrText.text = @"农林大学";
	_endCityText.text = @"临安";
	_endAddrText.text = @"万华广场";
    ...
    if (error == BMK_SEARCH_NO_ERROR)
    {
		...
		for (int i = 0; i < size; i++)
        {
            ...
            if(i==0){
                ...
                region.center.latitude  = plan.starting.location.latitude;
                
                region.center.longitude = plan.starting.location.longitude;
            ...
            if(i==0)
            {
                ...
                NSString *urlString =transitStep.instruction;
             
                NSError *error;
                NSRegularExpression *reg=	[NSRegularExpression 	regularExpressionWithPattern:@"(在.*下车,)(.+)(<.+>)(.*)" options:0 error:&error];
                
                	NSTextCheckingResult *firstMatch = [reg firstMatchInString:urlString options:0 range:NSMakeRange(0, [urlString length])];
                    NSRange resultRange = [firstMatch rangeAtIndex:2];
                    //从urlString中截取数据
                    NSString  *result1 = [urlString substringWithRange:resultRange];
                    resultRange = [firstMatch rangeAtIndex:4];
                    NSString *result2=[urlString substringWithRange:resultRange];
                    NSString *result= [result1 stringByAppendingString:result2];
            ...
            else if(i==size-1)
            {
                ...
                NSString *urlString =transitStep.instruction;
                NSError *error;
                NSRegularExpression *reg=	[NSRegularExpression 	regularExpressionWithPattern:@"(<.+>)" options:0 error:&error];
                
                NSTextCheckingResult *firstMatch = [reg firstMatchInString:urlString options:0 range:NSMakeRange(0, [urlString length])];
                NSRange resultRange = [firstMatch rangeAtIndex:0];
                //从urlString中截取数据
                NSString *result = [urlString stringByReplacingCharactersInRange:resultRange withString:@" "];
